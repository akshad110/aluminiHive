...
export const getAllJobPostings: RequestHandler = async (req, res) => {
  try {
    ...
    // Build query - students can see all active jobs
...
export const createJobPosting: RequestHandler = async (req, res) => {
  try {
    const { userId, batchId, maxApplications, ...jobData } = req.body;
    ...
    if (maxApplications === 0) {
      delete jobData.maxApplications;
    } else if (maxApplications) {
      jobData.maxApplications = maxApplications;
    }
