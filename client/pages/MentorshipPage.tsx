...
import { openMentorshipCall } from '@/utils/streamCall';
...
export default function MentorshipPage() {
  ...
  const fetchMentors = async () => {
    try {
      setLoading(true);
      const response = await fetch(apiUrl('/alumni?availableForMentoring=true'));
      if (!response.ok) {
  ...
  const fetchMentorshipRequests = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/mentorship/requests/student/${user._id}`));
  ...
  const startStudentAgoraCall = (type: 'video' | 'audio', alumniEmail: string, requestId: string) => {
    ...
    const channelName = `mentorship-${requestId}`;
    ...
    alert(
      type === 'video'
        ? 'Joining video call. Ask your mentor to start the call first if you are not connected yet.'
        : 'Joining voice call. Ask your mentor to start the call first if you are not connected yet.'
    );
  ...
  const checkPaymentStatus = async (requestId: string, alumniId: string) => {
    ...
    try {
      const response = await fetch(apiUrl(`/payment/mentorship/status/${user._id}/${alumniId}/${requestId}`));
      const data = await response.json();
  ...
  const initiatePayment = async (requestId: string, alumniId: string) => {
    ...
    const verifyMentorshipPayment = async (
      orderId: string,
      paymentId: string,
      signature: string
    ) => {
      const verifyResponse = await fetch(apiUrl('/payment/mentorship/verify'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (verifyData.success) {
        ...
        alert(
          verifyData.mock
            ? 'Payment simulated successfully (development mode). Video/voice calls are now unlocked.'
            : 'Payment successful! Video/voice calls are now unlocked.'
        );
        fetchMentorshipRequests();
      } else {
    ...
    try {
      const orderResponse = await fetch(apiUrl('/payment/mentorship/order'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!orderResponse.ok) {
        alert(orderData.details || orderData.error || 'Failed to create payment order. Please try again.');
        return;
      }
      
      if (orderData.success && orderData.mock) {
        await verifyMentorshipPayment(
          orderData.order.id,
          `pay_mock_${Date.now()}`,
          'mock_dev_signature'
        );
                        ...
                        {request.status === 'accepted' && (
                          <div className="border-t pt-3 mt-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="relative">
                                <Button
                                  size="sm"
                                  variant="outline"
                                  className={`text-xs ${hasPaidForRequest(request._id) ? '' : 'opacity-50 cursor-not-allowed'}`}
                                  disabled={!hasPaidForRequest(request._id)}
                                  onClick={() => handleStudentCommunication('video', request.alumniId?.email, request._id)}
                                >
                                  <Video className="h-3 w-3 mr-1" />
                                  {hasPaidForRequest(request._id) ? 'Join Video Call' : 'Video Call'}
                                  {!hasPaidForRequest(request._id) && <Lock className="h-3 w-3 ml-1" />}
                                </Button>
                              ...
                              <Button
                                ...
                                className={`text-xs ${hasPaidForRequest(request._id) ? '' : 'opacity-50 cursor-not-allowed'}`}
                                disabled={!hasPaidForRequest(request._id)}
                                onClick={() => handleStudentCommunication('voice', request.alumniId?.email, request._id)}
                              >
                                <Phone className="h-3 w-3 mr-1" />
                                {hasPaidForRequest(request._id) ? 'Join Voice Call' : 'Voice Call'}
                                {!hasPaidForRequest(request._id) && <Lock className="h-3 w-3 ml-1" />}
                              </Button>
                              {!hasPaidForRequest(request._id) && (
                                <div className="absolute -top-2 -right-2 bg-red-500 text-white text-xs px-1 rounded-full">
                            ...
                            <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
                              ...
                              {hasPaidForRequest(request._id)
                                ? 'Ask your mentor to start the call first, then click Join Video/Voice Call.'
                                : 'Pay ₹100 to unlock calls. Your mentor must start the call before you can join.'}
                            </div>
                          </div>
                        )}
