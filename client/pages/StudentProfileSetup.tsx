...
export default function StudentProfileSetup() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      if (savedData) {
        ...
        setExpectedGraduationYear(data.expectedGraduationYear || new Date().getFullYear() + 1);
        setBranch(data.branch || "");
        setMinor(data.minor || "");
        setGpa(data.gpa || "");
        setInterests(Array.isArray(data.interests) ? data.interests.join(', ') : (data.interests || ""));
        setCareerGoals(Array.isArray(data.careerGoals) ? data.careerGoals.join(', ') : (data.careerGoals || ""));
        setSkills(data.skills ? data.skills.split(',').map((s: string) => s.trim()).filter(Boolean) : []);
        setIsLookingForMentorship(data.isLookingForMentorship || false);
        setMentorshipInterests(Array.isArray(data.mentorshipInterests) ? data.mentorshipInterests.join(', ') : (data.mentorshipInterests || ""));
      }
    } else {
      navigate('/auth');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/students/user/${user?._id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        const profileKey = `student_profile_${user._id}`;
        const profileData = {
          studentId,
          currentYear: parseInt(currentYear),
          expectedGraduationYear: parseInt(expectedGraduationYear),
