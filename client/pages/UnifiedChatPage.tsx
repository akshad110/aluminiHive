...
export default function UnifiedChatPage({ onChatSelect }: UnifiedChatPageProps) {
  ...
  useEffect(() => {
    const handleChatMessageSent = (event: Event) => {
      const { chatId, type, content, createdAt } = (event as CustomEvent).detail;
      setChats((prev) =>
        prev.map((chat) =>
          chat.id === chatId && chat.type === type
            ? { ...chat, lastMessage: content, timestamp: createdAt }
            : chat
        )
      );
    };
    ...
    return () => window.removeEventListener("chatMessageSent", handleChatMessageSent);
  }, []);
