...
export default function EmbeddedChat({ chatId, type, onBack }: EmbeddedChatProps) {
  ...
  const mergeMessages = (nextMessages: Message[]) => {
    setMessages((prev) => {
      if (
        ...
        prev.every((message, index) => message._id === nextMessages[index]?._id)
      ) {
        return prev;
      }
      return nextMessages;
  ...
  useEffect(() => {
    ...
    const pollMessages = () => {
      if (type === "batch") {
        loadBatchMessages({ silent: true });
      } else if (actualUserId || chatId) {
        loadPersonalMessages({ silent: true });
      }
    };

    const intervalId = window.setInterval(pollMessages, 4000);
    return () => window.clearInterval(intervalId);
  }, [chatId, type, user?._id, actualUserId]);

  const loadBatchMessages = async (options?: { silent?: boolean }) => {
    if (!user?._id) return;
    ...
    try {
      ...
      const messagesResponse = await fetch(
        `/api/batches/${chatId}/chat/messages?userId=${user._id}`
      );
      if (messagesResponse.ok) {
        const messagesData = await messagesResponse.json();
        mergeMessages((messagesData.messages || []).map(normalizeMessage));
      }
    } catch (error) {
  ...
  const fetchBatchChat = async () => {
    try {
      ...
      await loadBatchMessages({ silent: true });
    } catch (error) {
  ...
  const loadPersonalMessages = async (options?: { silent?: boolean }) => {
    const otherUserId = actualUserId || chatId;
    ...
    try {
      ...
      const messagesResponse = await fetch(`/api/messages/${user._id}/${otherUserId}`);
      if (messagesResponse.ok) {
        const messagesData = await messagesResponse.json();
        mergeMessages((messagesData.messages || []).map(normalizeMessage));
      }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const sentMessage = normalizeMessage(result.data);
        setMessages((prev) =>
          prev.map((message) =>
            message._id === tempId ? sentMessage : message
          )
        );
        window.dispatchEvent(
          new CustomEvent("chatMessageSent", {
            detail: {
              chatId: type === "personal" ? actualUserId || chatId : chatId,
              type,
        ...
        window.dispatchEvent(new CustomEvent("refreshMessageLimit"));
      } else if (response.status === 429) {
        setMessages((prev) => prev.filter((message) => message._id !== tempId));
        const errorData = await response.json();
        alert(
          errorData.message ||
            "Daily message limit reached. Please upgrade to continue messaging."
        );
        window.dispatchEvent(new CustomEvent("refreshMessageLimit"));
      } else {
        setMessages((prev) => prev.filter((message) => message._id !== tempId));
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden">
      {/* Chat Header */}
      <div className="bg-white border-b p-3 sm:p-4 flex items-center justify-between shrink-0">
      ...
      <div
        ...
        className="flex-1 min-h-0 overflow-y-auto p-2 sm:p-4 space-y-3 sm:space-y-4 bg-gray-50 overscroll-contain"
      >
        {messages.length === 0 ? (
