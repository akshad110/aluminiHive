...
export default function RootLayout() {
  ...
  return (
    <div className={`flex min-h-dvh flex-col w-full ${isChatPage ? "h-dvh overflow-hidden" : ""}`}>
      <SiteHeader />
      <main className={`flex-1 pt-16 w-full ${isChatPage ? "min-h-0 overflow-hidden" : ""}`}>
        <Outlet />
      </main>
      {!isChatPage && <SiteFooter />}
    </div>
  );
}
